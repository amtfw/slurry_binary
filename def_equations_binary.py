...
def binaryslurryODEs(parameters, init_guess):
    ...
    z = domain.grid(0)
    # Problem
    problem = de.NLBVP(domain, variables=['T','Tz','Pz','rhoS', 'rhoL', 'uS', 'uS_z','uL','uL_z', 'fS', 'fS_z', 'mDotS','Xi','Xi_z'], ncc_cutoff=1e-9)
    ...
    problem.parameters['lm'] = lambda_mu
    ...
    problem.add_equation("Xi_z = Xi*uL/c4 - c5*Xi*Pz/T")
    problem.add_equation("dz(Tz) = c2*( heatPipe -mDotS/St - heatFriction -heatViscous - heatReaction ) ")
